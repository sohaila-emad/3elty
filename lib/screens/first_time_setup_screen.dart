...
class _FirstTimeSetupScreenState extends State<FirstTimeSetupScreen> {
  ...
  Future<void> _sendOtp() async {
    ...
    if (phone.length < 10) {
      _setError('يرجى إدخال رقم هاتف صحيح');
      return;
    }
    ...
    try {
      ...
      if (!isRegistered) {
        _setError('هذا الرقم الهاتفي غير مسجل باسم أي عائلة.\nاطلب من مسؤول النظام إضافتك أولاً.');
  ...
  Future<void> _verifyOtp() async {
    ...
    if (code.length != 6) {
      _setError('الرجاء إدخال الرمز المكون من 6 أرقام');
      return;
    }
    if (_verificationId == null) {
      _setError('انتهت صلاحية جلسة التحقق. يرجى إعادة الإرسال.');
      return;
    }
    ...
    try {
      ...
      _setError('رمز غير صالح. يرجى المحاولة مرة أخرى.');
      setState(() => _isLoading = false);
    }
  }
  ...
  Future<void> _savePin() async {
    ...
    if (pin1.length != 4) {
      _setError('يجب أن يتكون الرقم السري من 4 أرقام بالضبط');
      return;
    }
    if (pin1 != pin2) {
      _setError('لا تتطابق أرقام التعريف الشخصية');
      return;
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('الإعداد لأول مرة'),
        leading: _step > 0
            ? IconButton(
  ...
  Widget _buildPhoneStep() => Column(
    ...
    children: [
      const _StepHeader(
        ...
        subtitle: 'سنرسل رمز تحقق لتأكيد هويتك',
      ),
  ...
  Widget _buildOtpStep() => Column(
    ...
    children: [
      _StepHeader(
        ...
        title: 'ادخل رمز التأكيد',
        subtitle: 'أرسلت إلى ${_phoneCtrl.text.trim()}',
      ),
      const SizedBox(height: 28),
      TextField(
        controller: _otpCtrl,
      ...
      TextButton(
        ...
        child: Text(
          _resendCountdown > 0
              ? 'أعد إرسال الرمز في ${_resendCountdown}s'
              : 'إعادة إرسال الرمز',
          style: TextStyle(
  ...
  Widget _buildPinStep() => Column(
    ...
    children: [
      const _StepHeader(
        ...
        title: 'قم بتعيين رقم التعريف الشخصي المكون من 4 أرقام',
        subtitle: 'ستستخدم هذا الرقم السري كل 10 عمليات تسجيل دخول كإجراء أمني.',
      ),
      const SizedBox(height: 28),
      _PinInputField(controller: _pin1Ctrl, label: 'أدخل رمز التحقيق'),
      const SizedBox(height: 16),
      _PinInputField(controller: _pin2Ctrl, label: 'تأكيد رمز التحقيق'),
      const SizedBox(height: 24),
      ElevatedButton(
        ...
        child: _isLoading
            ? const SizedBox(
                ...
                child: CircularProgressIndicator(
                  strokeWidth: 2, color: Colors.white))
            : const Text('احفظ رمز التحقيق وادخل إلى التطبيق'),
      ),
    ],
  );
}
