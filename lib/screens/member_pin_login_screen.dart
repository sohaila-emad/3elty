...
class _MemberPinLoginScreenState extends State<MemberPinLoginScreen> {
  ...
  Future<void> _handleLogin() async {
    if (_isLocked) {
      _showError('الحساب مقفل. يرجى المحاولة لاحقاً.');
      return;
    }
    ...
    try {
      // ── المرحلة الأولى: التحقق من رقم الهاتف ─────────────────────────────
      if (!_isReturningUser) {
        final isRegistered = await _authService.isPhoneRegisteredByAdmin(phone);
        if (!isRegistered) {
          _showError('رقم الهاتف هذا غير مسجّل.\nاطلب من المسؤول إضافتك أولاً.');
        ...
        if (!isSetUp) {
          // أول مرة على هذا الجهاز → الذهاب لإعداد الـ PIN
          if (mounted) {
        ...
        // المستخدم موجود — إظهار حقل الـ PIN
        setState(() {
      ...
      // ── المرحلة الثانية: التحقق من الـ PIN ───────────────────────────────
      final pin = _pinController.text;
      if (pin.isEmpty || pin.length != 4) {
        _showError('يرجى إدخال الـ PIN المكوّن من 4 أرقام');
        setState(() => _isLoading = false);
        return;
      }
  ...
  void _startLockoutCountdown() {
    Future.doWhile(() async {
      ...
      if (remaining <= 0) {
        ...
        _showError('تم فتح الحساب. يمكنك المحاولة مجدداً.');
        return false;
      }
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  _isReturningUser ? 'أدخل الـ PIN' : 'دخول العضو',
                ...
                Text(
                  _isReturningUser
                      ? 'أدخل الـ PIN المكوّن من 4 أرقام للمتابعة'
                ...
                if (_isLocked) ...[
                  Container(
                    ...
                    child: Column(children: [
                      ...
                      const SizedBox(height: 8),
                      Text(
                        'حاول مجدداً بعد $_lockoutMinutesRemaining دقيقة',
                ...
                Container(
                  ...
                  child: TextField(
                    ...
                    enabled: !_isLocked && !_isReturningUser,
                    ...
                    decoration: InputDecoration(
                      ...
                      prefixIcon:
                          Icon(Icons.phone, size: 24, color: Colors.teal[700]),
                ...
                // قسم الـ PIN (للمستخدم الموجود فقط)
                if (_isReturningUser) ...[
                  Container(
                    padding: const EdgeInsets.all(16),
                    decoration: BoxDecoration(
                  ...
                  if (_failedAttempts > 0 && !_isLocked) ...[
                    Container(
                      ...
                      child: Text(
                        'المحاولات الفاشلة: $_failedAttempts / 5',
                        style: TextStyle(
                          fontSize: 14,
                          color: Colors.orange[900],
                          fontWeight: FontWeight.w600,
