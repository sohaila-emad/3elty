...
class _PinCheckScreenState extends State<PinCheckScreen> {
  ...
  Future<void> _handlePinSubmit() async {
    if (_isLocked) {
      _showError('الحساب مغلق. يرجى المحاولة مرة أخرى لاحقاً.');
      return;
    }

    final pin = _pinController.text;
    if (pin.isEmpty || pin.length != 4) {
      _showError('يرجى إدخال رقم التعريف الشخصي المكون من 4 أرقام');
      return;
    }
  ...
  void _startLockoutCountdown() {
    Future.doWhile(() async {
      ...
      if (remaining <= 0) {
        ...
        _showError('تم فتح الحساب. يمكنك المحاولة مرة أخرى');
        return false;
      }
