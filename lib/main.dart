...
// ─── HELPER: عرض عمر الطفل بشكل صحيح ─────────────────────────────────────────
// الأطفال يُخزَّن عمرهم دائماً بالشهور في قاعدة البيانات.
// هذه الدالة تحوّل القيمة المخزَّنة إلى نص صحيح للعرض.
//   age = 6  → "٦ أشهر"
//   age = 12 → "سنة واحدة"
//   age = 24 → "سنتان"
//   age = 36 → "٣ سنوات"
//   age = 18 → "سنة ونصف"  (المخزَّنة كـ 18 شهراً)
String formatChildAge(int ageStoredInMonths) {
  if (ageStoredInMonths < 12) {
    return '$ageStoredInMonths شهراً';
  } else if (ageStoredInMonths == 12) {
    return 'سنة واحدة';
  } else if (ageStoredInMonths == 24) {
    return 'سنتان';
  } else if (ageStoredInMonths % 12 == 0) {
    return '${ageStoredInMonths ~/ 12} سنوات';
  } else {
    final years = ageStoredInMonths ~/ 12;
    final months = ageStoredInMonths % 12;
    return '$years سنة و$months شهراً';
  }
}

/// يُرجع نص العمر المناسب لعرضه في الـ UI بناءً على نوع الملف الشخصي
String formatMemberAge(FamilyMember member) {
  if (member.profileType == ProfileType.child) {
    return formatChildAge(member.age);
  }
  return '${member.age} سنة';
}
  ...
  switch (type) {
    case ProfileType.child:
      return const [
        ...
        // ── تم إضافة وحدة الأدوية للطفل ──────────────────────────────────
        HealthModule(
          id: 'medications',
          title: 'أدوية الطفل',
          subtitle: 'تتبع أدوية الطفل وجرعاته اليومية',
            ...
            child: Column(
              ...
              children: [
                ...
                Row(children: [
                  ...
                  _ProfileChip(
                    // استخدم formatMemberAge للعرض الصحيح (أشهر/سنوات للطفل)
                    label: formatMemberAge(member),
