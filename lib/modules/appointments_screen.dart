...
class _AppointmentsScreenState extends State<AppointmentsScreen> {
        ...
        child: Column(
          ...
          children: [
            ...
            TextField(
              ...
              decoration: _buildInputDecoration('التاريخ', 'السنة-الشهر-اليوم'),
            ),
              ...
              child: ElevatedButton(
                onPressed: () async {
                  ...
                  try {
                    ...
                    final hasConflict = _appointments.any((appt) => appt.scheduledAt == dateCtrl.text);
                    if (hasConflict) {
                      _showError('لديكِ موعد آخر بالفعل في هذا التاريخ!');
                      return;
                    }
                    final apptId = '${widget.member.id}_${dateCtrl.text}_${titleCtrl.text.trim()}';
                    ...
                    // ── جدولة تذكيرات الموعد (يوم قبله + في يومه) ──────────
                    ...
                    messenger.showSnackBar(SnackBar(
                      ...
                      content: const Text('تمت جدولة الموعد وسيتم تذكيرك ✓', style: TextStyle(color: Colors.white)),
                    ));
                  } catch (e) {
                    if (!mounted) return;
                    _showError('فشل: $e');
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
                  ...
                  child: _appointments.isEmpty
                    ...
                    itemBuilder: (_, i) {
                      ...
                      return Material(color: Colors.white, borderRadius: BorderRadius.circular(12), child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        ...
                        Text('الموعد: ${appt.scheduledAt}', style: const TextStyle(fontSize: 13, color: AppColors.teal, fontWeight: FontWeight.w500)),
                      ])));
                    },
                  ),
                ),
      ...
      floatingActionButton: FloatingActionButton.extended(onPressed: _showAddAppointmentDialog, backgroundColor: AppColors.teal, foregroundColor: Colors.white, icon: const Icon(Icons.add_rounded), label: const Text('جدولة موعد')),
    );
  }
}
