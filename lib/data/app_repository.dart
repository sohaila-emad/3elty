...
class MedicationRecord {
  ...
  factory MedicationRecord.fromMap(Map<String, dynamic> m) => MedicationRecord(
    // Fix: guard against integer rowids or nulls coming back from SQLite
...
class AppRepository {
  ...
  /// Returns the generated String ID so callers can reference it immediately
  /// (e.g. for scheduling notifications or deleting the record later).
  Future<String> insertMedication(MedicationRecord r) async {
    final db = await _db;
    // Always generate a String ID — never rely on SQLite's integer rowid,
    // which comes back as null when the column type is TEXT.
    final id = _generateId();
    final mapWithId = r.toMap();
    mapWithId['id'] = id;
    await db.insert('medications', mapWithId,
        conflictAlgorithm: ConflictAlgorithm.replace);
    return id;
  }
