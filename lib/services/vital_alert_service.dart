...
/// Check a vital reading against thresholds
    ...
    final result = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text(
              'أدخلت القيمة $readingValue $unit لـ $vitalType.',
              textAlign: TextAlign.center,
              style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w500),
            ),
            const SizedBox(height: 8),
            const Text(
              'هذه القراءة في مستوى حرج. هل القراءة صحيحة؟',
              textAlign: TextAlign.center,
              style: TextStyle(
                fontSize: 14,
  ...
  Future<void> sendDangerAlert({
    ...
    try {
      ...
      await _sendLocalNotification(
        title: '⚠️ تنبيه: علامة حيوية حرجة',
        body: 'قراءة $vitalType ($readingValue $unit) في مستوى حرج!',
      ...
      for (final token in adminTokens) {
        await _sendPushNotification(
          token: token,
          title: '🚨 تنبيه عائلي: $memberName',
          body: '$vitalType = $readingValue $unit (حرج)',
          data: {
            'type': 'vital_alert',
            'member_id': memberId,
  ...
  Future<void> sendWarningAlert({
    ...
    try {
      await _sendLocalNotification(
        title: '⚠️ تحذير: علامة حيوية',
        body: 'قراءة $vitalType ($readingValue $unit) في منطقة التحذير.',
