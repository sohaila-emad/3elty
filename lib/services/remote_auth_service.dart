...
class RemoteAuthService {
  ...
  Future<String> signInWithPin({
    ...
    // نجيب الـ familyId من رقم التليفون مباشرة (مش محتاجين session موجودة)
    String? currentFamilyId = await familyId;

    // لو مفيش session → نجيب الـ familyId من members collection
    if (currentFamilyId == null) {
      final memberLookup = await _firestore
          .collection('members')
          .where('phone', isEqualTo: phone.trim())
          .limit(1)
          .get();
      if (memberLookup.docs.isEmpty) {
        throw Exception('رقم الهاتف غير مسجّل في أي عائلة.');
      }
      currentFamilyId = memberLookup.docs.first['family_id'] as String?;
      if (currentFamilyId == null) throw Exception('الحساب غير مرتبط بعائلة.');
    }
